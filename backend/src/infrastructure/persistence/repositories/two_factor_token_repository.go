...
func (r *TwoFactorTokenRepositoryImpl) MarkAsUsed(ctx context.Context, tokenID int) error {
	...
	return r.db.WithContext(ctx).Model(t).Updates(map[string]any{
		"is_used": true,
	}).Error
}
...
func (r *TwoFactorTokenRepositoryImpl) GetLastToken(ctx context.Context, userID int, mfaType int) (*models.TwoFactorToken, error) {
	var result models.TwoFactorToken
	err := r.db.WithContext(ctx).
