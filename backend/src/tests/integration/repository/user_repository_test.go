...
func (suite *UserRepositoryTestSuite) SetupTest() {
	...
	// Initialize the test users slice for tracking
	suite.testUsers = make([]*models.User, 0)
}

func (suite *UserRepositoryTestSuite) TearDownTest() {
	// Clean up only the users we created in the test
	ctx := context.Background()
	for _, user := range suite.testUsers {
		if user.ID > 0 {
			err := suite.userRepo.Delete(ctx, user.ID)
			if err != nil {
				suite.T().Logf("Warning: Failed to delete test user ID %d: %v", user.ID, err)
			}
		}
	}
	// Clear the testUsers slice
	suite.testUsers = nil
}
...
func (suite *UserRepositoryTestSuite) TestFindByID() {
	...
	// Track the created user
	suite.testUsers = append(suite.testUsers, originalUser)
	...
	assert.Equal(suite.T(), originalUser.FullName, foundUser.FullName)
...
func (suite *UserRepositoryTestSuite) TestList() {
	...
	password := "testpassword"
	hashedPassword, _ := bcrypt.GenerateFromPassword([]byte(password), bcrypt.DefaultCost)
	emailPrefix := fmt.Sprintf("listtest_%d_", time.Now().UnixNano())
	
	for i := 1; i <= 15; i++ {
		user := &models.User{
			Email:        fmt.Sprintf("%suser%d@example.com", emailPrefix, i),
			PasswordHash: string(hashedPassword),
			RoleID:       suite.testRole.ID,
	...
	filter := validator.UserListFilter{
		Page:     1,
		PageSize: 5,
		Search:   emailPrefix, 
	}
	users, totalPages, total, err := suite.userRepo.List(ctx, filter)

	// Assertions for first page
	assert.NoError(suite.T(), err)
	assert.Len(suite.T(), users, 5)
	assert.True(suite.T(), totalPages > 1)
	assert.Equal(suite.T(), 15, total, "Total count of test users should be 15")

	// Test second page
	filter.Page = 2
	usersPage2, _, _, err := suite.userRepo.List(ctx, filter)
	assert.NoError(suite.T(), err)
