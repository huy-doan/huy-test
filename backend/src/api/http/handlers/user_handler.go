...
// @Param role_id query integer false "Filter by role ID"
// @Param search query string false "Search by username, email or role name"
...
func (h *UserHandler) ListUsers(w http.ResponseWriter, r *http.Request) {
	...
	roleID := utils.GetQueryParamInt(r, "role_id", 0)
	search := utils.GetQueryParam(r, "search", "")
	sortField := utils.GetQueryParam(r, "sort_field", "")
	sortOrder := utils.GetQueryParam(r, "sort_order", "asc")
	...
	users, totalPages, total, err := h.userUsecase.ListUsers(r.Context(), filter)
	if err != nil {
