...
type MerchantManagementUsecase interface {
	ListMerchants(ctx context.Context, input *inputdata.MerchantListInputData) ([]*merchant.Merchant, int, int, error)
}
...
func (uc *ManageMerchantsUsecase) ListMerchants(ctx context.Context, input *inputdata.MerchantListInputData) ([]*merchant.Merchant, int, int, error) {
	const (
		defaultPage     = 1
		defaultPageSize = 10
	)
