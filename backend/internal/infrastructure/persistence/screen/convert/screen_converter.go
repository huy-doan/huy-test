...
// ToScreenDTO converts a Screen domain model to a ScreenDTO
func ToScreenDTO(screen *modelScreen.Screen) *dto.ScreenDTO {
	if screen == nil {
		return nil
	}

	result := &dto.ScreenDTO{
		ID:         screen.ID,
		Name:       screen.Name,
...
// ToScreenModel converts a ScreenDTO to a Screen domain model
func ToScreenModel(dtoObj *dto.ScreenDTO) *modelScreen.Screen {
	if dtoObj == nil {
		return nil
	}
...
func ToScreenDTOs(screens []*modelScreen.Screen) []*dto.ScreenDTO {
	if screens == nil {
		return nil
	}

	result := make([]*dto.ScreenDTO, len(screens))
...
func ToScreenModels(dtos []*dto.ScreenDTO) []*modelScreen.Screen {
	if dtos == nil {
		return nil
	}
