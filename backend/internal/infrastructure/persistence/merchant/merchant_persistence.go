...
func NewMerchantRepository(db *gorm.DB) merchantRepo.MerchantRepository {
...
func (r *MerchantRepositoryImpl) ListMerchants(ctx context.Context, params *inputdata.MerchantListInputData) ([]*merchantModel.Merchant, int, int, error) {
	query := r.db.WithContext(ctx).Model(&dto.MerchantDTO{})
	...
	merchants := make([]*merchantModel.Merchant, len(merchantDTOs))
