...
type InsertPayinTransactionTask struct {
	PaypayPayinTransactionRepo paypayRepo.PaypayPayinTransactionRepository
	appLogger                  logger.Logger
}

func NewInsertPayinTransactionTask(repo paypayRepo.PaypayPayinTransactionRepository, appLogger logger.Logger) *InsertPayinTransactionTask {
	return &InsertPayinTransactionTask{
		PaypayPayinTransactionRepo: repo,
		appLogger:                  appLogger,
	}
}
...
func (t *InsertPayinTransactionTask) Do(ctx context.Context, payinFileID int, records []map[string]string) error {
	...
	err = tx.Transaction(func(txCtx *gorm.DB) error {
		...
		for _, record := range records {
			...
			transactionAt, _ := parseDatetime(record["transaction_at"])
		...
		err := t.PaypayPayinTransactionRepo.BulkInsert(ctx, transactions)
		if err != nil {
			t.appLogger.ErrorWithContext("[InsertPayinTransactionTask] Error inserting transactions: %v", err)
			return err
		}
